	...
	function lgCheck() {
	  ...
	  function addLike(i) {
		if(this.lgCheck){
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/slog/slogLike",
				type : "POST",
				data : {slog_seq : i},
	  ...
	  function addCmt(i) {
		  ...
		  if(this.lgCheck){
			  ...
			  if(cmt != ""){
				  ...
				  $.ajax({
						url: "${pageContext.request.contextPath}/slog/slogCmt",
						type: "POST",
						data: {slog_seq : i, content : cmt},
						success: function(data){
							if(data === '성공'){
								location.reload();
							}
	  ...
	  function slogCmtLike(i) {
		if(this.lgCheck){
			...
			$.ajax({
				url: "${pageContext.request.contextPath}/slog/slogCmtLike",
				type: "POST",
				data: {slog_cmt_seq : i},
...
<body>
<form id="slogDetailForm">
<input type="hidden" name="lgcheck" id="lgcheck" value="${lgcheck }">
<input type="hidden" name="slog_seq" id="lgcheck" value="${slog.SLOG_SEQ }">
</form>
<div class="major">
	<div class="top">
		<div class="topF">
	...
	<div class="mid" >
		<div class="midT">
			...
			<c:choose>
				...
				<c:otherwise>
					<div class="editMenu" style="display: none;" id="editMenu">
						<div class="editSel" onclick="location.href='${cpath}/member/report/slogReport?kind=0&slog=${slog.SLOG_SEQ }'">신고하기</div>
					</div>
				</c:otherwise>
			</c:choose>
		</div>
