    ...
    <html lang="kr">
        ...
        <body>
            <div class="main" align="center">
                ...
                <form action="${cpath}/admin/sreport/sreportUpdate" method="post" target="_parents"  onsubmit="return update(this);">
                <input type ="hidden" value="${mOne.S_REPORT_SEQ}" name="s_report_seq">
                <input type ="hidden" value="${mOne.REPORT_CATE_SEQ}" name="report_cate_seq">
                <input type ="hidden" value="${mOne.KIND}" name="kind">
                <input type ="hidden" value="${mOne.BOARD_NO}" name="board_no">

                    <div class="table">
                        <div class="tb-1">
                            <div class="t1-title">회원닉네임</div>
                            <div class="t1-text"><a href="javascript:void(0);" onclick="opner('${cpath}/admin/member/content?m_seq=${mOne.MEMSEQ}')"  style="color: black;">${mOne.M_NICKNAME}</a></div>
                        </div>
                        <div class="tb-1">
                            <div class="t1-title">신고사유</div>
                            <div class="t1-text">${mOne.REPORT_CATE_NAME} </div>
                        </div>
                           <div class="tb-1">
                            <div class="t1-title">신고점수</div>
                            <div class="t1-text">${mOne.SCORE} </div>
                        </div>
                        <div class="tb-1">
                            <div class="t1-title">이메일</div>
                            <div class="t1-text">${mOne.MEMAIL} </div>
                        </div>
                      	<c:if test="${mOne.KIND ==1}">
	                      	<div class="tb-1">
		                        <div class="t1-title"> <a href="javascript:void(0);" onclick="opner('${cpath }/slog/slogDetail?slog_seq=${mOne.SLOG_SEQ }')" style="text-decoration: none; color:gray; cursor: pointer; " >여행기/여행기댓글 </a></div><!-- 여행기 상세 경로 넣기 -->
		                            <div class="t1-text">${mOne.SCLCT}</div>
	                       	</div>
                      	</c:if>
                      	<c:if test="${mOne.KIND !=1}">
	        				<div class="tb-1">
		                        <div class="t1-title"> <a href="javascript:void(0);" onclick="opner('${cpath }/slog/slogDetail?slog_seq=${mOne.SLOG_SEQ}')" style="text-decoration: none; color: rgba(58, 58, 58, 0.4); cursor: pointer; " >여행기/여행기댓글 </a></div><!-- 여행기 상세 경로 넣기 -->
		                             <div class=t1-textt><textarea name="travel_comments" readonly style="width: 500px; height: 45px;">${mOne.INTRO}${mOne.SCLCT}</textarea></div>
	                       	</div>
                        </c:if>
                        <div class="tb-1">
                         ...
                         <div class="tb-2">
                            <div class="t2-text">
                                <textarea name="content" readonly >${mOne.CONTENT}</textarea>
                            </div>
                        </div>
