                ...
                <div class="main-detail">
			                            ...
			                            <div class="normal">
			                                ...
			                                <c:choose>
				                                ...
				                                <c:otherwise>
				                                	<div class="rsub-last" id="tip-add${rc.RV_CMT_SEQ}">
					                                    <div class="report">
					                                        <a style="cursor: pointer;" onclick="window.open('${cpath}/member/report/review?kind=1&rv_seq=${rc.RV_CMT_SEQ }','신고하기','width = 800px, height=600px, top=300px, left=300px, scrollbars=yes')"><span>신고하기</span></a>
					                                    </div>
					                                    <img onclick="XTip(${rc.RV_CMT_SEQ})" src="https://assets.triple.guide/images/ico_tooltip_delete.png">
					                                </div>
				                                </c:otherwise>
                    ...
                    <script>
                        ...
                        function updateCMT(v){
                        	var content = document.getElementById("content"+v).textContent;
                        	var contentHTML = document.getElementById("content"+v).innerHTML;
                        	
                        	if(contentHTML.includes('<span>')){
	                        	
	                        	var spanArea = '';
	                        	
	                        	var start = contentHTML.indexOf('<span>');
	                        	var end = contentHTML.indexOf('</span>');
	                        	
	                        	if(start !== -1 && end !== -1){
	                        		spanArea = contentHTML.substring(start, end + 7);
	                        	}
	                        	
	                        	
	                        	//m_seq추출
	                        	var startM = contentHTML.indexOf('?m_seq=');
	                        	var endM = contentHTML.indexOf('">');
	                        	
	                        	var m_seqArea = '';
	                        	
	                        	if(startM !== -1 && endM !== -1){
	                        		m_seqArea = contentHTML.substring(startM + 7, endM);
	                        	}
	                        	
	                        	
	                        	//@tag추출
	                        	var startT = contentHTML.indexOf('@');
	                        	var endT = contentHTML.indexOf('&nbsp');
	                        	
	                        	var tagArea = '';
	                        	
	                        	if(startT !== -1 && endT !== -1){
	                        		tagArea = contentHTML.substring(startT, endT + 5);
	                        	}
	                        	
	                        	
	                        	var tagbox = document.getElementById("tagbox");
	                        	tagbox.innerHTML = tagArea + "<input type='button' value='X' onclick='cancel();'>";
	                        	
	                        	
	                        	//content추출
	                        	var startC = contentHTML.indexOf('</span>');
	                        	
	                        	var content = '';
	                        	if(startC !== -1){
	                        		content = contentHTML.substring(startC + 7);
	                        	}
